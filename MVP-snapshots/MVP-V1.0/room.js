...
const roomBgm = new Audio("素材/音频/audio/music/room-bgm-loop.mp3");
...
boxItems.forEach((item) => {
  item.addEventListener("click", () => {
    ...
    boxPhotoImage.src = "素材/图片【移除背景】/纸箱_全班合照_正面_桌面.png";
  ...
  item.addEventListener("click", () => {
    ...
    if (item.dataset.bagItem !== "password-book") {
      ...
      const bagItemThought = {
        ...
        badge: "我带着可威风了。",
        "pencil-case": "这只铅笔盒，装下了好多次放学前的匆忙。",
...
document.querySelector("[data-mute]").addEventListener("click", (event) => {
  const muted = event.currentTarget.textContent === "静音";
  event.currentTarget.textContent = muted ? "有声" : "静音";
