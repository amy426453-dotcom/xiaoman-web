...
const MODULE_IMAGE_PRELOADS = {
  bag: [
    "素材/图片【移除背景】/书包_旧书包_打开_桌面.png",
    "素材/图片【移除背景】/书包_旧书包_完成_桌面.png",
    "素材/图片【移除背景】/书包_红领巾_展开_桌面.png",
    "素材/图片【移除背景】/书包_喜羊羊铅笔盒_打开_桌面.png",
    "素材/图片【移除背景】/书包_新华字典_打开_桌面.png",
    "素材/图片【移除背景】/书包_跳绳_展开_桌面.png",
    "素材/图片【移除背景】/书包_同桌纸条_正面_桌面_带字.png",
    "素材/图片【移除背景】/书包_密码本_打开_桌面.png",
  ],
  computer: ["素材/图片【移除背景】/电脑_旧电脑_开机_桌面.png"],
  drawer: [
    "素材/图片【移除背景】/场景_儿童房背景_抽屉打开_桌面.webp",
  ...
  box: [
    ...
    "素材/图片【移除背景】/纸箱_搬家纸箱_合上_桌面.png",
    "素材/图片【移除背景】/纸箱_实验中学校服_展开_桌面（带姓名贴）.png",
    "素材/图片【移除背景】/纸箱_黑色双肩书包_拉链微开_桌面.png",
    "素材/图片【移除背景】/纸箱_全班合照_背面_桌面.png",
  ],
};
const MODULE_AUDIO_PRELOADS = {
  bag: [...Object.values(bagSfx), bagOpenSfx, bagCloseSfx, passwordButtonSfx, passwordErrorSfx],
  computer: [computerBootSfx, computerCloseSfx, computerClickSfx, matchSuccessSfx, matchErrorSfx, matchHintSfx, ...Object.values(farmActionSfx)],
  drawer: [...Object.values(drawerSfx), drawerPullSfx],
  mp3: [mp3Audio, mp3StartSfx, mp3ButtonSfx],
  height: [],
  box: [...Object.values(boxSfx), photoFlipSfx, boxOpenSfx, boxCloseSfx, boxRecheckSfx],
};
const MODULE_MEDIA_CONTAINERS = {
  bag: [bagItemsPanel],
  computer: [computerPreviewCard, computerModule],
  drawer: [drawerItemsPanel, drawerPhoto],
  mp3: [mp3Player],
  height: [],
  box: [boxItemsPanel, boxPhotoDetail],
};
const preloadedModules = new Set();
const scheduledModules = new Set();
const warmingImages = new Set();

Object.values(MODULE_AUDIO_PRELOADS).flat().forEach((audio) => { audio.preload = "none"; });
...
function warmImage(src) {
  if (!src || warmingImages.has(src)) return;
  warmingImages.add(src);
  const image = new Image();
  image.onload = image.onerror = () => warmingImages.delete(src);
  image.src = src;
}

function preloadModule(region) {
  if (!region || preloadedModules.has(region)) return;
  preloadedModules.add(region);
  MODULE_MEDIA_CONTAINERS[region]?.forEach(loadDeferredImages);
  MODULE_IMAGE_PRELOADS[region]?.forEach(warmImage);
  MODULE_AUDIO_PRELOADS[region]?.forEach((audio) => {
    audio.preload = "auto";
    audio.load();
  });
  if (region === "computer" && matchBoard && !matchBoard.childElementCount) buildMatchBoard();
}

function scheduleModulePreload(region) {
  if (!region || preloadedModules.has(region) || scheduledModules.has(region)) return;
  scheduledModules.add(region);
  const run = () => {
    scheduledModules.delete(region);
    preloadModule(region);
  };
  if ("requestIdleCallback" in window) window.requestIdleCallback(run, { timeout: 1200 });
  else window.setTimeout(run, 600);
}

function scheduleFollowingModule(region) {
  const currentIndex = MODULE_ORDER.indexOf(region);
  const followingRegion = MODULE_ORDER[currentIndex + 1];
  if (followingRegion) scheduleModulePreload(followingRegion);
}
...
function openRegion(region) {
  ...
  preloadModule(region);
  scheduleFollowingModule(region);
