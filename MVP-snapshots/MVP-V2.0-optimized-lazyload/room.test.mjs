...
test("hidden module media waits for staged preloading", () => {
  const deferredImages = html.match(/<img[^>]+data-src="素材\//g) ?? [];
  assert.ok(deferredImages.length >= 30, "hidden module images should not use eager src attributes");
  assert.match(html, /data-mp3-audio[^>]*preload="none"/);
  assert.match(script, /function loadDeferredImages\(container\)/);
  assert.match(script, /function preloadModule\(region\)/);
  assert.match(script, /function scheduleModulePreload\(region\)/);
});

test("the current and next story modules are warmed before interaction", () => {
  assert.match(script, /function openRegion\(region\) \{[\s\S]*preloadModule\(region\);[\s\S]*scheduleFollowingModule\(region\);/);
  assert.match(script, /function updateNextHotspot\(\)[\s\S]*scheduleModulePreload\(next\);/);
  assert.match(script, /function scheduleFollowingModule\(region\)/);
  assert.match(script, /requestIdleCallback/);
  assert.match(script, /const MODULE_IMAGE_PRELOADS = \{/);
  assert.match(script, /const MODULE_AUDIO_PRELOADS = \{/);
});
